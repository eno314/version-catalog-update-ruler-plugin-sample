...
class RssParser {
    ...
    fun parseRss20(xmlString: String): Rss20FetchDto {
        ...
        val channelTitle = (xpath.evaluate("//channel/title/text()", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "サイトのタイトル"
        val channelLink = (xpath.evaluate("//channel/link/text()", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "https://example.com"
        val channelDescription = (xpath.evaluate("//channel/description/text()", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "サイトの概要説明"
        ...
        for (i in 0 until itemNodes.length) {
            ...
            val thumbnailUrl = (itemXpath.evaluate("media:thumbnail/@url", itemNode, XPathConstants.STRING) as? String)?.takeIf { it.isNotEmpty() }
        ...
        return Rss20FetchDto(
            ...
            items = items.takeIf { it.isNotEmpty() } ?: listOf(
                Rss20ItemDto(
    ...
    fun parseAtom(xmlString: String): AtomFetchDto {
        ...
        val feedTitle = (xpath.evaluate("//feed/title/text()", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "サイトのタイトル"
        val feedLink = (xpath.evaluate("//feed/link[@rel='alternate']/@href", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() }
            ?: (xpath.evaluate("//feed/link/@href", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "https://example.com"
        val feedSubtitle = (xpath.evaluate("//feed/subtitle/text()", document, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "サイトの概要説明"
        ...
        for (i in 0 until entryNodes.length) {
            ...
            val author = (entryXpath.evaluate("author/name/text()", entryNode, XPathConstants.STRING) as String).takeIf { it.isNotEmpty() } ?: "著者名"
            val thumbnailUrl = (entryXpath.evaluate("media:thumbnail/@url", entryNode, XPathConstants.STRING) as? String)?.takeIf { it.isNotEmpty() }
        ...
        return AtomFetchDto(
            ...
            entries = entries.takeIf { it.isNotEmpty() } ?: listOf(
                AtomEntryDto(
